...
size_t index_helper(const std::vector<T>& elements, const C& val, size_t left, size_t right, size_t mid){
    if(elements[mid] == val)
        return mid;
    if(elements[mid] < val && elements[mid+1] > val)
        return mid+1;

    if(val < elements[mid])
        return index_helper(elements, val, left, mid, (left+mid)/2);
    else
        return index_helper(elements, val, mid+1, right, (mid+right+1)/2);
...
size_t insertion_idx(const std::vector<T>& elements, const C& val)
{
    ...
    return index_helper(elements, val, 0, size-1, (size-1)/2);
}
