...
RGBAPixel gradientColorPicker::operator()(int x, int y)
{
    ...
    int distance = abs(x - xcoord) + abs(y - ycoord);
    if (distance > rad)
        color = end;
    else{
        color.red = start.red - (distance * start.red / rad) + (distance * end.red / rad);
        color.blue = start.blue - (distance * start.blue / rad) + (distance * end.blue / rad);
        color.green = start.green - (distance * start.green / rad) + (distance * end.green / rad);
    }
