...
void HuffmanTree::buildTree(const vector<Frequency>& frequencies)
{
    ...
    while((singleQueue.size()+mergeQueue.size()) >= 2){
      // cout<<"bulding"<<endl;
      TreeNode* tempL = removeSmallest(singleQueue, mergeQueue);
      TreeNode* tempR = removeSmallest(singleQueue, mergeQueue);

      TreeNode* newnode = new TreeNode((tempL->freq).getFrequency() + (tempR->freq).getFrequency());
      newnode->left = tempL;
      newnode->right = tempR;
...
HuffmanTree::TreeNode* HuffmanTree::readTree(BinaryFileReader& bfile)
{
  ...
  bool flag = bfile.getNextBit();
  if(flag){
    char c = bfile.getNextByte();
    Frequency f = Frequency(c, 0);
    TreeNode* ret = new TreeNode(f);
    return ret;
  }else{
    TreeNode* ret = new TreeNode(0);
    ret->left = readTree(bfile);
    ret->right = readTree(bfile);
    return ret;
  }
