...
PNG brighten(PNG original, int amount)
{
    ...
    for (size_t yi = 0; yi < original.height(); yi++) {
        for (size_t xi = 0; xi < original.width(); xi++) {
            ...
            temp_red = (*original(xi,yi)).red + amount;
            temp_green =( *original(xi,yi)).green + amount;
            temp_blue = (*original(xi,yi)).blue + amount;
            
            if (temp_red > 255)
	      {original(xi,yi)->red = 255;}
            else
	      {original(xi,yi)->red = temp_red;}
           
            if (temp_green > 255)
	      {original(xi,yi)->green = 255;}
            else
	      {original(xi,yi)->green = temp_green;}
            	
	    if (temp_blue > 255)
	      {original(xi,yi)->blue = 255;}
            else
	      {original(xi,yi)->blue = temp_blue;}
...
PNG blendImages(PNG firstImage, PNG secondImage)
{
  ...
  if (firstImage.width() > secondImage.width())
    new_width = firstImage.width();
  else
    new_width = secondImage.width();

  if (firstImage.height() > secondImage.height())
    new_height = firstImage.height();
  else
    new_height = secondImage.height();

 firstImage.resize(new_width, new_height);
 secondImage.resize(new_width, new_height);

 for (size_t yi = 0; yi < new_height; yi++) {
   for (size_t xi = 0; xi < new_width; xi++) {
