...
class KDTree
{
  ...
  private:
    ...
    int partition(int left, int right, int pivot, int dimension);

    int getDis(Point<Dim> const & first, Point<Dim> const & second) const;

    bool is_range_dimension(Point<Dim> const & cur,  Point<Dim> const & target, int dimension, int radius) const;

    void findhelper(const Point<Dim>& query, Point<Dim>& currentBest, int left, int right, int cur, int& radius, bool & flag) const;
