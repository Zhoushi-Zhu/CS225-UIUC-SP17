...
class BinaryTree
{
  ...
  private:
    ...
    void mirror_helper(Node* subRoot) const;

    bool order_helper(const Node* subRoot) const;

    void path_helper(int* s, const Node* subRoot, int length) const; 

    void printStack(int* s, int length) const;

    void dist_helper(int &sum, const Node* subRoot, int length) const;
